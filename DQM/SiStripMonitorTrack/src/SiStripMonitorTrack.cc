...
void SiStripMonitorTrack::bookSubDetMEs(std::string& name){
  ...
  if(Trend_On_){
    ...
    if (TrendsVsLS) {
      ...
      theSubDetMEs.nClustersTrendOnTrackLS = bookMETrend("TH1nClustersOn", completeName.c_str());
      completeName = "Trend_TotalNumberOfClusters_OffTrackVsLumisection"  + subdet_tag;
      theSubDetMEs.nClustersTrendOffTrackLS = bookMETrend("TH1nClustersOff", completeName.c_str());
    }
    else {
