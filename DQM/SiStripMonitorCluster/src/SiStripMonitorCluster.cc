...
void SiStripMonitorCluster::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (std::map<std::string, std::vector< uint32_t > >::const_iterator iterLayer = LayerDetMap.begin();
	...
	if (found_layer_me) {
          if(TrendsVsLS) fillLayerMEsLS(layer_single, cluster_properties, aLS);
...
void SiStripMonitorCluster::createLayerMEs(std::string label, int ndets , DQMStore::IBooker & ibooker) {
  ...
  if(layerswitchcluswidthon) {
    ...
    if (createTrendMEs) { 
        if(TrendsVsLS) layerMEs.LayerClusterWidthTrendLS=bookMETrend("TH1ClusterWidth", hidmanager.createHistoLayer("Trend_ClusterWidthvsLumisection","layer",label,"").c_str() , ibooker);
  	else layerMEs.LayerClusterWidthTrend=bookMETrend("TH1ClusterWidth", hidmanager.createHistoLayer("Trend_ClusterWidth","layer",label,"").c_str() , ibooker);
    }
  }
  ...
  if(layerswitchclusnoiseon) {
    ...
    if (createTrendMEs) {
	if(TrendsVsLS) layerMEs.LayerClusterNoiseTrendLS=bookMETrend("TH1ClusterNoise", hidmanager.createHistoLayer("Trend_ClusterNoisevsLumisection","layer",label,"").c_str() , ibooker);
    	else layerMEs.LayerClusterNoiseTrend=bookMETrend("TH1ClusterNoise", hidmanager.createHistoLayer("Trend_ClusterNoise","layer",label,"").c_str(), ibooker); 
    }
  }
  ...
  if(layerswitchcluschargeon) {
    ...
    if (createTrendMEs) {
	if(TrendsVsLS) layerMEs.LayerClusterChargeTrendLS=bookMETrend("TH1ClusterCharge", hidmanager.createHistoLayer("Trend_ClusterChargevsLumisection","layer",label,"").c_str(), ibooker);
    	else layerMEs.LayerClusterChargeTrend=bookMETrend("TH1ClusterCharge", hidmanager.createHistoLayer("Trend_ClusterCharge","layer",label,"").c_str(), ibooker); 
    }
  }
  ...
  if(layerswitchclusstonon) {
    ...
    if (createTrendMEs) {
	if(TrendsVsLS) layerMEs.LayerClusterStoNTrendLS=bookMETrend("TH1ClusterStoN", hidmanager.createHistoLayer("Trend_ClusterSignalOverNoisevsLumisection","layer",label,"").c_str(), ibooker);
    	else layerMEs.LayerClusterStoNTrend=bookMETrend("TH1ClusterStoN", hidmanager.createHistoLayer("Trend_ClusterSignalOverNoise","layer",label,"").c_str(), ibooker); 
    }
  }
  ...
  if(layerswitchlocaloccupancy) {
    ...
    if (createTrendMEs) {
	if(TrendsVsLS) layerMEs.LayerLocalOccupancyTrendLS=bookMETrend("TH1ModuleLocalOccupancy", hidmanager.createHistoLayer("Trend_ClusterLocalOccupancyvsLumisection","layer",label,"").c_str(), ibooker);
    	else layerMEs.LayerLocalOccupancyTrend=bookMETrend("TH1ModuleLocalOccupancy", hidmanager.createHistoLayer("Trend_ClusterLocalOccupancy","layer",label,"").c_str(), ibooker); 
    }
  }
...
void SiStripMonitorCluster::createSubDetMEs(std::string label , DQMStore::IBooker & ibooker ) {
  ...
  if (subdetswitchtotclusprofonvsLS){
    ...
    subdetMEs.SubDetTotClusterProfLS = ibooker.bookProfile(HistoName,HistoName,
							    Parameters.getParameter<int32_t>("Nbins"),
							    Parameters.getParameter<double>("xmin"),
							    Parameters.getParameter<double>("xmax"),
							    100, //that parameter should not be there !?
